...
class Wait(Base, db.Model):
    ...
    def to_dict(self):
        return dict(old_wait_sql=self.old_wait_sql, old_wait=self.old_wait, old_wait_time=self.old_wait_time,
                    old_wait_mysql=self.old_wait_mysql, new_wait_sql=self.new_wait_sql, new_wait=self.new_wait,
                    new_wait_time=self.new_wait_time, new_wait_mysql=self.new_wait_mysql)
