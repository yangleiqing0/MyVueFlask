...
class Mysql(BaseModel, db.Model):
    ...
    def to_dict(self):
        return dict(id=self.id, name=self.name, ip=self.ip, port=self.port, user=self.user, password=self.password,
                    db_name=self.db_name, description=self.description, user_id=self.user_id)
